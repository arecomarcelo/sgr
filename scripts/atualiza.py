...
def atualizar_ambiente():
    ...
    try:
        ...
        try:
            subprocess.check_call(
                [sys.executable, "-m", "pip", "uninstall", "-y", "streamlit-aggrid"]
            )
        except subprocess.CalledProcessError:
            print(
                "streamlit-aggrid não estava instalado ou houve erro na desinstalação."
            )
        ...
        subprocess.check_call(
            [sys.executable, "-m", "pip", "install", "streamlit-aggrid==0.3.4"]
        )
        ...
        streamlit_version = next(
            (p for p in installed_packages if p.startswith("streamlit==")),
            "streamlit não encontrado",
        )
        aggrid_version = next(
            (p for p in installed_packages if p.startswith("streamlit-aggrid==")),
            "streamlit-aggrid não encontrado",
        )
