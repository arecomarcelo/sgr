    ...
    def load_data(data_inicial=None, data_final=None):
        ...
        try:
            with st.spinner("Carregando dados..."):
                if not data_inicial:
                    data_inicial = (date.today() - relativedelta(days=1)).strftime(
                        "%Y-%m-%d"
                    )
                if not data_final:
                    data_final = (date.today() - relativedelta(days=1)).strftime(
                        "%Y-%m-%d"
                    )
    ...
    def calculate_totals(self, data):
        ...
        try:
            if data.empty or "Status" not in data.columns:
                return {"total_enviados": 0, "total_errados": 0, "total_geral": 0}
    ...
    def render_totals(self, totals, df):
        ...
        try:
            ...
            with container:
                ...
                with col1:
                    st.metric(label="Total Enviados", value=totals["total_enviados"])
                with col2:
                    st.metric(label="Total Errados", value=totals["total_errados"])
                with col3:
                    st.metric(label="Total Geral", value=totals["total_geral"])
                with col_button:
                    st.write("")
                    if not df.empty:
        ...
        try:
            ...
            for index, row in df_formatted.iterrows():
                ...
                if "Vencimento" in df_formatted.columns:
                    df_formatted.at[index, "Vencimento"] = self.format_date(
                        row["Vencimento"]
                    )
