...
def download_excel(df):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        for idx, col in enumerate(df_formatted.columns):
            max_length = (
                max(df_formatted[col].astype(str).apply(len).max(), len(str(col))) + 2
            )
...
def display_totals(totals, df):
    ...
    with container:
        ...
        with col1:
            st.metric(
                label="Total Produtos", value=format_number(totals["total_produtos"])
            )
        with col2:
            st.metric(label="Total Custo", value=format_currency(totals["total_custo"]))
        with col3:
            st.metric(label="Total Venda", value=format_currency(totals["total_venda"]))
        with col_button:
